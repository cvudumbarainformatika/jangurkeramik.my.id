<template>
  <div>
    ...
    >
      <div 
        class="bg-white rounded-2xl shadow-2xl overflow-hidden w-full max-w-md border border-gray-100 transition-all duration-500 ease-out max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div v-if="!isLoggedIn" class="p-6">
          ...
          <!-- Tampilan untuk set password dengan UI yang lebih profesional -->
          <div v-else-if="currentView === 'set-password'" class="animate-fade-in">
            ...
            <div class="text-center mb-6">
              <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-500/20">
                <AppIcon name="key" class="text-white" size="lg" />
              </div>
              <h4 class="text-xl font-medium text-gray-800 mb-2">Atur Password untuk Akun Anda</h4>
              <p class="text-gray-600 mb-4">
                Dengan mengatur password, Anda dapat login menggunakan email dan password selain menggunakan {{ getProviderName(socialProvider) }}.
              </p>
            </div>
            ...
            <form @submit.prevent="setPasswordForSocialAccount" class="flex flex-col gap-4">
              ...
              <!-- Error message -->
              <div v-if="passwordError" class="text-red-500 text-sm p-2 bg-red-50 rounded-lg flex items-start">
                <AppIcon name="alert-circle" class="mr-1 mt-0.5 flex-shrink-0" size="sm" />
