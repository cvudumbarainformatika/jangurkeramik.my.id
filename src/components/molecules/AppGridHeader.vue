<template>
  <div class="flex w-full ">
    <div class="desktop-only w-full flex flex-wrap items-center justify-between">
    ...
    <!-- Mobile View -->
    <div class="mobile-only w-full mb-4">
      <div class="flex items-center justify-between space-x-4">
        <!-- Sort Dropdown -->
        <div v-if="showSort" class="flex">
          <div class="relative">
            <select 
            v-model="sortOptionModel" 
            @change="$emit('sort', sortOptionModel)"
        ...
        <!-- View Toggle -->
        <div v-if="showViewToggle" class="flex-none items-center space-x-2 bg-white border border-gray-300 rounded-lg p-1">
          <button 
            @click="$emit('view-mode-change', 'grid')"
            :class="[
              'p-1 rounded transition-colors',
